...
/**
 * `payment-methods unionpay-status [pm_id]` — query the current UnionPay
 * card-binding status for a payment method, once (no polling).
 *
 * This is the status-check half of `payment-methods add --payment-brand
 * unionpay` / `unionpay-enroll`: it reads GET /payment-methods/<pm_id> and
 * prints the current status, then exits. Callers (e.g. the agent
 * orchestrator) poll this on their own cadence after starting enrollment with
 * `payment-methods unionpay-enroll`, instead of letting the CLI block for 60
 * seconds.
 *
 ...
 * Not advertised in the SKILL/README (mirrors dropin-status).
 */
export function registerUnionpayStatusCommand(
  parent: Command,
  deps: { apiClient: ApiClient },
): void {
  const cmd = parent
    .command('unionpay-status [pm_id]')
    .description('Query the current UnionPay card-binding status for a payment method (single check)')
