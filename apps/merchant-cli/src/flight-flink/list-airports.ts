...
import { attachSchemaHelp, flightListAirportsSchema } from '../verb-schema.js';
import { type Deps, num, render, resolveApiKey } from './_helpers.js';

/** `flight-flink list-airports` — paged airport dictionary. */
export function registerListAirportsCommand(parent: Command, deps: Deps): void {
  const cmd = parent
    .command('list-airports')
    .description('Paged airport dictionary')
  ...
  attachSchemaHelp(cmd, flightListAirportsSchema);
  ...
  cmd.action(async () => {
    ...
    const spinner = format === 'json' ? null : createSpinner('Fetching airports...');
