...
export const SERVICE_REGISTRY: ServiceCapability[] = [
  {
    ...
    provider: 'elife',
    cli_noun: 'ride-elife',
    verbs: ['quote', 'book', 'get', 'cancel', 'update', 'trip-status', 'list-orders'],
    verb_descriptions: {
      ...
      update: 'Modify a booked ride: trip details and/or passenger info.',
      'trip-status': 'Retrieve status and driver location points for one leg of a multi-leg ride.',
    ...
    workflow: [
      ...
      'get (poll for status)',
      'update (optional)',
      'trip-status (optional, multi-leg rides)',
      'cancel (optional)',
    ],
    since: '2026-06-01',
    discovery: {
  ...
  {
    // 与后端 services/merchant_service/discovery/catalog.py 的 flight-flink 条目 + provider
    // schema flight-flink.json 的 service_id 保持一致。Agent 拼命令用 cli_noun="flight-flink"。
    service_id: 'svc_01J0HT5FLINK0001',
    name: 'Flight booking (Flink)',
    description:
      'International flight booking: airport/city resolution, one-way/round-trip/multi-city search with journeyId relay, price verification, two-step create-then-pay ticketing, order polling, plus change (rebooking) and refund flows.',
    category: 'flight',
    version: '1.0.0',
    provider: 'flink',
    cli_noun: 'flight-flink',
    verbs: [
      'find-airport',
      'list-airports',
      'list-airlines',
    ...
    verb_descriptions: {
      'find-airport': 'Resolve free-text into city/airport candidates.',
      'list-airports': 'Paged airport dictionary.',
      'list-airlines': 'Paged airline dictionary.',
      'list-nationalities': 'Nationality list.',
      search: 'Search flights (one-way/round-trip/multi-city) with journeyId relay.',
      'more-offers': 'More fare offers for a priceKey.',
      verify: 'Verify the latest price before booking; returns the authoritative product_token.',
      'create-order': 'Create a flight order without charging (locks the fare).',
      'pay-order': 'Settle a created order by --order-no (triggers ticketing).',
      'get-order': 'Query order status (poll ticketing 5→8).',
      'cancel-order': 'Cancel an un-ticketed order (with refund).',
      'list-orders': 'List flight orders (local store, no upstream call).',
      'change-search': 'Search rebook-eligible flights.',
      'change-apply': 'Submit a change request (pending review).',
      'change-detail': 'Change request detail.',
      'change-pay': 'Pay the change fee and trigger change ticketing.',
      'change-cancel': 'Cancel a change request (immediate terminal).',
      'refund-apply': 'Submit a refund request (pending review).',
      'refund-detail': 'Refund request detail.',
      'refund-confirm': 'Confirm ("1") or cancel ("2") a refund.',
    },
    workflow: [
      'find-airport',
      'search (relay journey-id for round-trip/multi-city)',
      'verify',
      'create-order (locks the fare, no charge)',
      'pay-order (settles + ticketing)',
      'get-order (poll until TICKETED)',
      'change-* / refund-* / cancel-order (optional)',
    ],
    since: '2026-07-09',
    discovery: {
      help_command: 'agenzo-merchant-cli flight-flink --help',
    },
  },
];
